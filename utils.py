...
def get_ap_score_alpha_beta(y_true, y_scores):
    """
    Get average precision score between 2 arrays by computing class probabilities.

    Args:
        y_true: batch of true labels, shape (N, 20) for 20 classes.
        y_scores: batch of model outputs, shape (N, 40), where the first 20 elements
                  are alpha and the last 20 elements are beta.
    ...
    alpha = y_scores[:, :20]  # First 20 elements are alpha
    beta = y_scores[:, 20:]  # Last 20 elements are beta


    # Calculate the probability for each class as alpha / (alpha + beta)
    probabilities = alpha / (alpha + beta)

    # Loop through each sample to calculate the average precision score
    for i in range(y_true.shape[0]):
        scores += average_precision_score(y_true=y_true[i], y_score=probabilities[i])
