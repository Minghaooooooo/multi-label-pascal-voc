...
def main(data_dir, model_name, num, lr, epochs, batch_size = 16, download_data = False, save_results=False):
    ...
    model.fc = torch.nn.Linear(num_ftrs, 20*2)
    model.to(device)
    optimizer = optim.SGD([
...
#                                              ]),
    ...
    # Define the size of the subset (1/10 of the original dataset)
    subset_size = len(dataset_train) // 10

    # Generate random indices for the subset
    subset_indices = random.sample(range(len(dataset_train)), subset_size)

    # Create the subset of the original training dataset
    dataset_train_small = Subset(dataset_train, subset_indices)

    # Create a DataLoader for the small training dataset
    train_loader_small = DataLoader(dataset_train_small, batch_size=batch_size, num_workers=4, shuffle=True)
    ...
    trn_hist, val_hist = train_model(model, device, optimizer, scheduler, train_loader_small, valid_loader, model_dir, num, epochs, log_file)
    torch.cuda.empty_cache()
...
if __name__ == '__main__':
   main('../data/', "resnet18", num=3, lr = [1e-2, 1e-2], epochs = 200, batch_size=16, download_data=True, save_results=True)
